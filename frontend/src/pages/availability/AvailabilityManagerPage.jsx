import {useState,useEffect,Fragment} from 'react';import StatusBadge from '../../components/common/StatusBadge';import api from '../../services/api';
...
export default function AvailabilityManagerPage(){
  ...
  useEffect(()=>{api.get('/availability/mine').then(r=>{const sl=r.data?.slots||DEMO;setSaved(sl);setSelected(new Set(sl.filter(s=>s.status!=='cancelled').map(s=>`${s.dayOfWeek}-${s.startTime}`)));}).catch(()=>{setSaved(DEMO);setSelected(new Set(DEMO.filter(s=>s.status!=='cancelled').map(s=>`${s.dayOfWeek}-${s.startTime}`)));}).finally(()=>setLoading(false));}, []);
  ...
  const save=async()=>{setSaving(true);const slots=[...selected].map(k=>{const[day,time]=k.split('-');const h=parseInt(time);return{dayOfWeek:day,startTime:pad(h),endTime:pad(h+1)};});try{const r=await api.post('/availability/mine',{slots});const sl=r.data?.slots||slots;setSaved(sl);setSelected(new Set(sl.filter(s=>s.status!=='cancelled').map(s=>`${s.dayOfWeek}-${s.startTime}`)));}catch{}setSaving(false);};
  return(
    <div className="page">
      ...
      <div className="container section">
        ...
        <div className="card"><div className="card-body">
          ...
          {loading?<div style={{textAlign:'center',padding:'2rem',color:'var(--text3)'}}>Loading…</div>:(
            <div className="avail-grid">
              ...
              {HOURS.map(h=><Fragment key={h}><div className="cal-time-label" style={{height:38}}>{pad(h)}</div>{DAYS.map(d=>{const k=`${d}-${pad(h)}`;const isBkd=booked.has(k);const isOn=selected.has(k);return<button key={k} className={`avail-slot ${isBkd?'booked-slot':isOn?'on':'off'}`} onClick={()=>toggle(d,h)}>{isBkd?'●':isOn?'✓':''}</button>;})}</Fragment>)}
            </div>
          )}
        </div></div>
        <div className="card" style={{marginTop:'1.25rem'}}><div className="card-body">
          ...
          :<div className="table-wrap"><table><thead><tr><th>Day</th><th>Start</th><th>End</th><th>Status</th></tr></thead><tbody>{saved.map(s=><tr key={s.id||`${s.dayOfWeek}-${s.startTime}-${s.endTime}`}><td>{s.dayOfWeek}</td><td>{s.startTime}</td><td>{s.endTime}</td><td><StatusBadge status={s.status==='available'?'Available':'Booked'}/></td></tr>)}</tbody></table></div>}
        </div></div>
      </div>
    </div>
  );
