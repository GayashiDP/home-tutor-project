...
export default function TransactionHistoryPage(){
  const [txns,setTxns]=useState([]); const [loading,setLoading]=useState(true); const [expandedId,setExpandedId]=useState(null);
  useEffect(()=>{api.get('/payments/history').then(r=>setTxns(normalizeTransactions(r.data))).catch(()=>setTxns(DEMO)).finally(()=>setLoading(false));}, []);
          ...
          {txns.map(t=><div key={t.id} className="card"><div className="card-body">
            ...
            {expandedId===t.id&&<div style={{marginTop:'1rem',paddingTop:'1rem',borderTop:'1px solid var(--border)'}}>
              <div className="receipt-card">
                ...
                {[['Receipt ID',t.receiptId],['Student',t.studentName],['Tutor',t.tutorName],['Subject',t.subject],['Session Date',t.sessionDate],['Duration',`${t.startTime}–${t.endTime}`],['Amount',fmtCurr(t.amount)],['Status',t.displayStatus],['Issued',fmtDate(t.paidAt)]].map(([l,v])=><div key={l} className="receipt-row"><span style={{color:'var(--text3)'}}>{l}</span><strong>{v}</strong></div>)}
              </div>
              <button className="btn btn-secondary btn-sm" style={{marginTop:'.75rem'}}>⬇ Download PDF Receipt</button>
            </div>}
          </div></div>)}
        </div>}
      </div>
