...
const normalizeSlotsResponse = data => Array.isArray(data) ? data : data?.slots || [];
...
export default function TutorDetailPage() {
  ...
  useEffect(()=>{
    Promise.all([
      ...
      api.get(`/availability/tutors/${id}`).catch(()=>({data:DEMO_SLOTS})),
  ...
  const doBook=async(e)=>{
    e.preventDefault(); if(!bSubj)return; setSubmitting(true);
    try{ await api.post('/bookings',{tutorId:tutor.id,slotId:selSlot.id,subject:bSubj,note:bNote}); }catch{}
    setBooked(true); setSelSlot(null); setSubmitting(false);
  };
      ...
      <div className="container" style={{paddingTop:'1.35rem',paddingBottom:'2rem'}}>
        ...
        <div className="card" style={{marginBottom:'1.35rem'}}>
          <div className="tutor-detail-hero">
            ...
            <div className="tutor-rate-badge"><div className="rate-num">${tutor.hourly_rate}</div><div className="rate-label">per hour</div></div>
          </div>
          {booked&&<div className="alert alert-success" style={{margin:'1rem 1.35rem'}}>✅ <div><strong>Booking request submitted!</strong><p style={{marginTop:'.1rem',fontSize:'.82rem'}}>Your session request is pending confirmation from {tutor.name}.</p></div></div>}
        </div>
        <div className="grid-2" style={{marginBottom:'1.35rem'}}>
          ...
          <div className="card"><div className="card-body">
            ...
            <div className="grid-2">
              {[{v:slots.filter(s=>s.status==='available').length,l:'Available Slots'},{v:slots.filter(s=>s.status==='booked').length,l:'Booked Slots'},{v:(tutor.subjects||[]).length,l:'Subjects'},{v:tutor.reviewCount||reviews.length,l:'Reviews'}].map(s=>(
        ...
        <div className="card" style={{marginBottom:'1.35rem'}}><div className="card-body">
          ...
          <p style={{fontSize:'.8rem',color:'var(--text2)',marginBottom:'.9rem'}}>🟢 Available to book · 🔵 Already booked · Click a green slot to request a session</p>
            ...
            {HOURS.map(h=>(
              <React.Fragment key={h}>
                ...
                {DAYS.map(d=>{
                  ...
                  const cls=slot?.status==='available'?'available':slot?.status==='booked'?'booked':'';
                  const canBook=user?.role==='Student'&&cls==='available';
                  return<button key={`${d}${h}`} className={`cal-slot ${cls}`} disabled={!canBook} onClick={()=>{if(canBook){setSelSlot(slot);setBooked(false);setBConf(false);setBSubj(getSubjectName(tutor.subjects?.[0])||'');setBNote('');}}}>
                    {cls==='available'?'Book':cls==='booked'?'Booked':''}
                  </button>;
                })}
              </React.Fragment>
            ))}
            ...
            <form onSubmit={doBook}>
              <div className="modal-body">
                <div className="grid-2" style={{marginBottom:'.9rem'}}>
                  {[{l:'Tutor',v:tutor.name},{l:'Day',v:selSlot.dayOfWeek},{l:'Time',v:`${selSlot.startTime}–${selSlot.endTime}`},{l:'Rate',v:`$${tutor.hourly_rate}/hr`}].map(i=>(
